...
OpenPost currently supports publishing to X, Mastodon, Bluesky, Threads, LinkedIn, Facebook Pages, and TikTok. Instagram and YouTube are visible in provider discovery as planned adapters only.
...
- **Planned providers are discovery-only** — adding Instagram or YouTube to provider app config fails until their adapters are implemented.
...
- Deployment details still matter. Threads, Facebook, and TikTok depend on a public media URL, and LinkedIn depends heavily on granted app permissions.
