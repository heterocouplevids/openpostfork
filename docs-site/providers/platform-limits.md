...
OpenPost currently supports publishing to X, Mastodon, Bluesky, Threads, LinkedIn, Facebook Pages, Instagram Business, and TikTok. YouTube is visible in provider discovery as a planned adapter only.
...
- **Planned providers are discovery-only** — adding YouTube to provider app config fails until its adapter is implemented.
