...
OpenPost currently supports publishing to X, Mastodon, Bluesky, Threads, LinkedIn, and TikTok. Instagram, Facebook, and YouTube are visible in provider discovery as planned adapters only.
...
- **Planned providers are discovery-only** — adding Instagram, Facebook, or YouTube to provider app config fails until their adapters are implemented.
...
- Deployment details still matter. Threads and TikTok depend on a public media URL, and LinkedIn depends heavily on granted app permissions.
