...
- Add Playwright smoke tests for marketing, login, onboarding, composer, scheduling, accounts, settings, and media. Coverage is now in place for marketing, docs audience separation, app settings/billing/MCP activity, provider discovery, custom Mastodon connect, plan onboarding, publication handoff, account-specific composer previews, and media-library upload/listing.
