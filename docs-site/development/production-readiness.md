...
- Start with safe semantic tools: list workspaces, list accounts, create draft, set post renditions, upload media from URL, schedule post, cancel post, get post status, suggest next slot. The remote MCP foundation now supports workspace/account listing, draft creation, destination-specific rendition updates, guarded URL media upload, quota-checked scheduling, post status reads, scheduled-post queue inspection/cancellation, and next-slot suggestions.
