...
- Require auth for remote MCP, scope sessions, log tool calls, and expose revocation in settings. Tool-call logging is now persisted in `mcp_tool_calls`; session scoping and settings revocation are still next.
...
8. Start MCP with authenticated remote metadata and safe read/create/schedule tools. Remote auth, workspace listing, account listing, guarded URL media upload, draft creation, scheduled posting, status reads, scheduled-post cancellation, and next-slot suggestions are in place.
