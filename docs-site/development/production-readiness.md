...
- Add quota enforcement for account connection and team invites. Media upload quota enforcement is in place for monthly uploaded bytes and stored bytes, and scheduled-post quota enforcement is in place for single posts and threads.
...
- Store local subscription state and entitlement snapshots; do not call Polar on every request.
...
6. Add usage tables and API boundary checks. Monthly usage counters, media quota enforcement, and scheduled-post quota enforcement are in place; account and publishing-worker enforcement are next.
