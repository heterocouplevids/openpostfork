...
- Start with safe semantic tools: list workspaces, list accounts, create draft, upload media from URL, schedule post, cancel post, get post status, suggest next slot. The remote MCP foundation now supports workspace/account listing, draft creation, quota-checked scheduling, post status reads, and scheduled-post cancellation.
...
8. Start MCP with authenticated remote metadata and safe read/create/schedule tools. Remote auth, workspace listing, account listing, draft creation, scheduled posting, status reads, and scheduled-post cancellation are in place.
