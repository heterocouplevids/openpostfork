...
- CLI account and media commands: `openpost account list|disconnect`, `openpost account connect <platform>` (stubbed — OAuth lives on the server), `openpost media upload|list`. The `--accounts` picker resolves platform aliases (`x`, `linkedin`, `x:@username`) and account IDs against the workspace's social accounts, with a friendly disambiguation hint when a platform has multiple accounts. Accountpicker has table-driven unit tests covering the empty / single / multiple-match paths.
- CLI posting commands: `openpost post create|list|view|update|delete`, `openpost thread create`, and `openpost jobs list`. Posting supports workspace resolution, account aliases, natural-language schedules, local media upload, JSON output, and confirmation prompts for parsed natural schedules.
